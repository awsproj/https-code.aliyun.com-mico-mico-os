...
run:
	/Users/snowyang/workspace/tools/debuger/source/openocd/src/openocd -s $(SOURCE_ROOT) -f $(OPENOCD_CFG_PATH)interface/$(JTAG).cfg -f $(OPENOCD_CFG_PATH)$(HOST_OPENOCD)/$(HOST_OPENOCD).cfg -c init -c "reset" -c shutdown $(DOWNLOAD_LOG) 2>&1 && $(ECHO) Reset complete && $(ECHO_BLANK_LINE) || $(ECHO) Reset failed. See build/openocd_log.txt for detail.
