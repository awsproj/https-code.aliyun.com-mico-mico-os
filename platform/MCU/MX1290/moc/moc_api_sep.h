...
typedef struct {
	...
	int (*RsaSSL_Sign)(const byte* in, word32 inLen, byte* out, word32 outLen, RsaKey* key, CyaSSL_RNG* rng);
	int (*RsaPrivateKeyDecode)(const byte* input, word32* inOutIdx, RsaKey* key, word32 length);
}extra_crypto_api_t;
